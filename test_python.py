...
def _set_language(input_folder, output_folder):
    ...
    for character in characters:
        webURL = request.urlopen(f"https://www.dnd5eapi.co/api/races/{character['race']}")
        response = webURL.read()
        encoding = webURL.info().get_content_charset('utf-8')
...
_set_race("./names.json", "./races.json")
_set_language("./races.json", "./languages.json")
_set_race_proficiencies("./languages.json","./proficiencies.json")
_set_attributes("./proficiencies.json", "./attributes.json")
